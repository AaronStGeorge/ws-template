...
- Decisions and instructive pivots live here, inline — there are no ADR files. Record them when their lessons help explain the current system, especially when a choice is hard to reverse, surprising without context, or shaped by a real trade-off. Give each lesson as much space as its explanatory value earns; omit dead-end history that does not clarify the system. Preserve an overturned decision as `Previously: … — abandoned because …` rather than erasing it. Design carries the current strategy and the reasoning that shaped it; version control carries chronology and deeper forensics.
